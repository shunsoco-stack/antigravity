...
def _parse_tickers(raw_value):
    if not raw_value:
        return []
    return [ticker for ticker in re.split(r"[,\s]+", raw_value.strip()) if ticker]
...
def _passes_filters(data, min_per, max_per, min_pbr, max_pbr, min_div, max_div):
    val_per = data.get("PER (Trailing)")
    has_per_limit = min_per > 0 or max_per > 0
    if val_per is not None:
        if min_per > 0 and val_per < min_per:
            return False
        if max_per > 0 and val_per > max_per:
            return False
    elif has_per_limit:
        return False
    ...
    if val_pbr is not None:
        if min_pbr > 0 and val_pbr < min_pbr:
            return False
        if max_pbr > 0 and val_pbr > max_pbr:
            return False
    elif has_pbr_limit:
        return False
    ...
    if val_div is not None:
        ...
        if min_div > 0 and val_div_percent < min_div:
            return False
        if max_div > 0 and val_div_percent > max_div:
            return False
    elif has_div_limit:
        return False
...
def main():
    ...
    tab_fetch, tab_screen = st.tabs(["銘柄情報取得", "スクリーニング"])

    with tab_fetch:
        st.subheader("銘柄情報取得")
        st.caption("銘柄コードをカンマ区切りで入力してください。例: 7203, 9432, AAPL")

        ticker_input = st.text_input("銘柄コード", key="ticker_input")
        fetch_button = st.button("情報取得", type="primary", key="fetch_button")
    ...
    with tab_screen:
        ...
        search_button = st.button("スクリーニング実行", type="primary", key="screen_button")
        ...
        if search_button:
            ...
            for i, ticker in enumerate(tickers):
                ...
                if data:
                    ...
                    if _passes_filters(data, min_per, max_per, min_pbr, max_pbr, min_div, max_div):
                        results.append(data)
                        # リアルタイムでテーブルを更新
