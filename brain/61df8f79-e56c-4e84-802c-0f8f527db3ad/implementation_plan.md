...
- Update `get_stock_info` to include key `'Type'` with value `info.get('quoteType')`.
- In `main` function, when iterating through tickers:
...
- Remove the duplicate `get_stock_info` function and import/use `stock_tool.get_stock_info` instead.
