...
### 1. JPX List Retrieval (`stock_tool.py`)
- Added `get_all_jpx_tickers()` function.
...
### 3. Dependencies
- Added `openpyxl` and `xlrd` to `requirements.txt` to support reading the JPX Excel file.
...
-   **Market Scope**: Only stocks from **Prime**, **Standard**, and **Growth** markets are screened. ETFs, REITs, and other funds are excluded.
-   **Processing Time**: The tool scans the target list (approx. 3900 companies). This takes a significant amount of time.
